...
<?php TableOfContents(__FILE__, 4); ?>
        ...
        <p>The ITS tools are distributed under the terms of the GNU Public License <a href="http://www.gnu.org/licenses/gpl.html">GPL</a>.
		We have integrated modified versions of sources taken from other open-source projects such as Divine, VIS or LTSmin. See <a href="itstools.php#sec:ack">Acknowledgements section</a>.
		</p>
        <p>&nbsp;</p>
	...
	<ul>
	<li><a href="itstools.php#sec:modinst">for ITS Modeler eclipse front-end</a>
	</li><li><a href="itstools.php#sec:cldl">for ITS tools command-line</a>
	</li><li><a href="itstools.php#sec:library">for source distributions of libits and related tools</a>
	</li>
	</ul>
...
If you would like to use the ITS-tools as command-line tools without the front-end, please see <a href="#sec:command">command line section.</a><br/>
...
<p>
	If you don't already have graphviz, download it from <a
		href="http://www.graphviz.org/Download.php">http://www.graphviz.org/Download.php</a>.
...
<p>The other packages necessary for <a href="gal.php">GAL</a>,<a href="composite.php">ITS</a> or <a href="coloane.php">Time Petri
...
<p> We are grateful to <a href="http://www.jetbrains.com">JetBrains</a> and their open-source project support policy, that allows us to use the excellent
continuous integration server <a href="http://www.jetbrains.com/teamcity/">Teamcity</a> (see our setup at <a href="http://teamcity-systeme.lip6.fr">teamcity-systeme.lip6.fr</a>).
...
Example ITS models are also provided in the front-end.
		...
		<a href="gal.php">GAL</a>,<a href="composite.php">ITS Composite</a> or <a href="coloane.php">Time Petri
		nets</a>  or <a href="analysis.php">CTL/LTL</a>
...
<ul>
	...
	-t {CAMI|PROD|ROMEO|ITSXML|ETF|DVE|GAL|DLL|NDLL} : specifies format of the input model file :
		<ul>
			<li>CAMI : CAMI format (for P/T nets) is the native Petri net format of <a href="http://move.lip6.fr/software/CPNAMI/MANUAL_SERV/index.html">CPN-AMI</a>
            </li><li>PROD : PROD format (for P/T nets) is the native format of <a href="http://www.tcs.hut.fi/Software/prod/">PROD</a>
            </li><li> ROMEO : an XML format (for Time Petri nets) that is the native format of <a href="http://romeo.rts-software.org/">Romeo</a>
            </li><li> ITSXML : a native XML format (for ANY kind of ITS) for this tool. These files allow to point to other files and are used as main file for composite definitions. See <a href="https://projets-systeme.lip6.fr/trac/research/libddd/browser/libits/trunk/Samples/ITSXML/philo4/modelMain.xml">this example</a>, the list of formalism/format supported is described <a href="https://projets-systeme.lip6.fr/trac/research/libddd/browser/libits/trunk/src/ITSModelXMLLoader.cpp">here</a>. 
            </li><li> ETF : <a href="http://fmt.cs.utwente.nl/tools/ltsmin/etf.html">Extended Table Format</a> is the native format used by LTSmin, built from many front-ends.
            </li><li> DVE : Divine is a modelling language similar to Promela. Input file should be in <a href="http://divine.fi.muni.cz/manual.html#quick-guide-the-dve-specification-language">Divine format</a>.
            </li><li> GAL : Guarded Action Language. Input file should be in <a href="gal.php">GAL syntax</a>.
            ...
            </li><li> NDLL : same as DLL, but expect input formatted as size:lib.so. See <a href="https://projets-systeme.lip6.fr/trac/research/libddd/browser/libits/trunk/demo/philoSumo.cpp">demo/ folder</a> for a usage example. Both DLL and NDLL are used to inject of arbitrary C++ ITS types into the ITSModel.
			</li>
		</ul>
	</li>
	</ul>
	...
	A single ordering file may contain order of many ITS types. <a href="https://projets-systeme.lip6.fr/trac/research/libddd/browser/libits/trunk/tests/test_models/train4.ord">An example order file.</a> See also the output of "--dump-order".
	</p>
	...
	<a href="https://projets-systeme.lip6.fr/trac/research/NEOPPOD/wiki/Framekit_guide_stand_alone#a4HeuristicDescription">PNXDD</a> tool which embeds various heuristics that produce these files.
	A Petri net loaded with a hierarchical order is re-encoded internally as a composition of subnets. See <a href="http://pagesperso-systeme.lip6.fr/Fabrice.Kordon/pdf/2012-toponc-pnxdd.pdf">this March 2012 TopNoc paper</a> that describes these heuristics in detail.
	<br/>
	...
	<p>
	For models that use GAL or Divine formalisms, we have adapted <a href="http://web.eecs.umich.edu/~karem/papers/Aloul-GLSVLSI03.pdf">Aloul et al's FORCE heuristic.</a>. 
	...
	<p>
	For more details on this recursive encoding, see the <a href="http://ceur-ws.org/Vol-726/04-componet-2.pdf">2011 ITS definition paper</a> where Scalar and Circular composite and their encodings  are defined (page 8-9).
	</p>
...
<h4><a name="sec:cluse"></a>2.1 Options specific to its-reach </h4>
	...
	See <a href="https://projets-systeme.lip6.fr/trac/research/libddd/browser/libits/trunk/tests/tests.def">this page</a> for invocation examples.
	...
	<p>	
	The first step is to download and build libddd from source, we recommend directly from svn, following <a href="libddd.php#sec:svn">this procedure</a>.
...
<p>Note that to build from the svn, you need autotools and to invoke
	...
	Remember, configure --help is your friend, and you can also have a look at the configuration settings of our <a href="https://teamcity.rsr.lip6.fr/">continuous integration server</a>
...
<br />
<a href="files/antlr.sh">Download as a shell script</a>
<br />
...
<div class="toplink" align="right"><a href="#toc">Start of page <img alt="" src="images/up.gif" width="13" height="12" border="0"></a></div>
	
<h3><a name="sec:libitsdl"></a>2. Source install of libITS</h3>
<br />
<a href="files/libits.sh">Download as a shell script</a>
<br />
...
<a href="files/ctlits.sh">Download as a shell script</a>
<br />
...
<br />
<a href="files/ltlits.sh">Download as a shell script</a>
<br />
